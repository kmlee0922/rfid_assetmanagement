...
public class Frame1 {
	...
	private void initialize() {
		...
		gl_configuration_panel.setHorizontalGroup(gl_configuration_panel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_configuration_panel.createSequentialGroup().addGroup(gl_configuration_panel
						.createParallelGroup(Alignment.LEADING)
						.addGroup(gl_configuration_panel.createSequentialGroup().addGap(27)
								.addGroup(gl_configuration_panel.createParallelGroup(Alignment.LEADING)
										.addComponent(ant_power_panel, GroupLayout.PREFERRED_SIZE, 468,
												GroupLayout.PREFERRED_SIZE)
										.addComponent(ver_panel, GroupLayout.PREFERRED_SIZE, 468,
												GroupLayout.PREFERRED_SIZE)))
						.addGroup(gl_configuration_panel.createSequentialGroup().addGap(166)
								.addComponent(config_btnRead, GroupLayout.PREFERRED_SIZE, 77,
										GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(ComponentPlacement.UNRELATED).addComponent(config_btnWrite,
										GroupLayout.PREFERRED_SIZE, 77, GroupLayout.PREFERRED_SIZE)))
						.addContainerGap(36, Short.MAX_VALUE)));
		gl_configuration_panel.setVerticalGroup(gl_configuration_panel.createParallelGroup(Alignment.TRAILING)
				.addGroup(Alignment.LEADING, gl_configuration_panel.createSequentialGroup().addGap(25)
						.addComponent(ver_panel, GroupLayout.PREFERRED_SIZE, 64, GroupLayout.PREFERRED_SIZE).addGap(18)
						...
						.addGroup(gl_configuration_panel.createParallelGroup(Alignment.BASELINE)
								.addComponent(config_btnRead).addComponent(config_btnWrite))
		...
		table.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "Tag ID (HEX)", "TEXT" }));
		...
		lk_kill_pw_comboBox.setModel(new DefaultComboBoxModel(new String[] { "ACCESSIBLE", "ALWAYS ACCESSIBLE",
				"SECURED ACCESSIBLE", "ALWAYS NOT ACCESSIBLE", "NO CANGE" }));
		...
		lk_access_pw_comboBox.setModel(new DefaultComboBoxModel(new String[] { "ACCESSIBLE", "ALWAYS ACCESSIBLE",
				"SECURED ACCESSIBLE", "ALWAYS NOT ACCESSIBLE", "NO CANGE" }));
		...
		lk_epc_bank_comboBox.setModel(new DefaultComboBoxModel(new String[] { "ACCESSIBLE", "ALWAYS ACCESSIBLE",
				"SECURED ACCESSIBLE", "ALWAYS NOT ACCESSIBLE", "NO CANGE" }));
		...
		lk_tid_bank_comboBox.setModel(new DefaultComboBoxModel(new String[] { "ACCESSIBLE", "ALWAYS ACCESSIBLE",
				"SECURED ACCESSIBLE", "ALWAYS NOT ACCESSIBLE", "NO CANGE" }));
		...
		lk_user_bank_comboBox.setModel(new DefaultComboBoxModel(new String[] { "ACCESSIBLE", "ALWAYS ACCESSIBLE",
				"SECURED ACCESSIBLE", "ALWAYS NOT ACCESSIBLE", "NO CANGE" }));
		...
		lk_btnLock.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				// if (MessageBox.Show(message, "ggg ", MessageBoxButtons.YesNo,
				...
				// DialogResult.Yes)
				{
					...
					LockTag(killpermissions, accesspermissions, epcpermissions, tidpermissions, userpermissions,
							password);
				}
			}
		});
		...
		lk_btnKill.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				// if (MessageBox.Show(message, "dngkgkgkg", MessageBoxButtons.YesNo,
				// MessageBoxIcon.Information, MessageBoxDefaultButton.Button2) ==
				// DialogResult.Yes)
				{
					String password = lk_pw_editorPane.getText().toUpperCase();
		...
		comboBox_7.setModel(new DefaultComboBoxModel(
				new String[] { "INVENTORY_S0", "INVENTORY_S1", "INVENTORY_S2", "INVENTORY_S3", "SELECTED" }));
		comboBox_7.setBounds(126, 10, 146, 21);
		panel_8.add(comboBox_7);

		JComboBox comboBox_8 = new JComboBox();
		comboBox_8.setModel(
				new DefaultComboBoxModel(new String[] { "ASLINVA_DSLINVB", "ASLINVA_NOTHING", "NOTING_DSLINVB",
						"NSLINVS_NOTHING", "DSLINVB_ASLINVA", "DSLINVB_NOTHING", "NOTING_ASLINVA", "NOTING_NSLINVS" }));
		...
		comboBox_11.setModel(new DefaultComboBoxModel(new String[] { "INVENTORY_SESSION_S0", "INVENTORY_SESSION_S1",
				"INVENTORY_SESSION_S2", "INVENTORY_SESSION_S3" }));
		comboBox_11.setBounds(124, 35, 198, 21);
		panel_9.add(comboBox_11);

		JComboBox comboBox_12 = new JComboBox();
		comboBox_12.setModel(
				new DefaultComboBoxModel(new String[] { "INVENTORY_SESSION_TARGET_A", "INVENTORY_SESSION_TARGET_B" }));
	...
	public void Inventory(InventoryAction action) {
		...
		/*
		 * if (Reader2 != null) { //if (Reader.IsHandling) { if (action ==
		 * InventoryAction.Start) { Reader2.ReadTagId(TagType.ISO18000_6C_GEN2,
		 * ReadType.MULTI); lblstatusLabel.setText("Run Inventory"); } else {
		 * 
		 * Reader2.StopOperation(); lblstatusLabel.setText("Ready");
		 * 
		 * //bool returnvalue = reader.StopOperationToSync(); //if (returnvalue) //{ //
		 * this.tslblStatus.Text = "Ready";
		 * 
		 * // Inventory(Action.Start); //} //else //{ // this.tslblStatus.Text =
		 * "Stop Failed"; //} } } }
		 */
	}
	...
	private void GetConfiguration() {
		...
		/*
		 * if (Reader2 != null) { //if (Reader.IsHandling && !Reader.IsInventorying) {
		 * //this.Cursor = Cursors.WaitCursor;
		 * 
		 * //// Version - reader //reader.GetVersion(0); Thread.Sleep(300);
		 * Reader2.GetVersion(0);
		 * 
		 * try { Thread.sleep(10); } catch (InterruptedException e1) { // TODO
		 * Auto-generated catch block e1.printStackTrace(); } //// Version - H/W
		 * //reader.GetVersion(5); Thread.Sleep(300); Reader2.GetVersion(5); try {
		 * Thread.sleep(10); } catch (InterruptedException e1) { // TODO Auto-generated
		 * catch block e1.printStackTrace(); } //// Version - Root
		 * //reader.GetVersion(2); Thread.Sleep(300); Reader2.GetVersion(2); try {
		 * Thread.sleep(10); } catch (InterruptedException e1) { // TODO Auto-generated
		 * catch block e1.printStackTrace(); }
		 * 
		 * //// Version - reader //this.lblVersionReader.Text =
		 * reader.GetVersionToSync(0); Thread.Sleep(300); //// Version - H/W
		 * //this.lblVersionHardware.Text = reader.GetVersionToSync(5);
		 * Thread.Sleep(300); //// Version - Root //this.lblVersionRoot.Text =
		 * reader.GetVersionToSync(2); Thread.Sleep(300);
		 * 
		 * 
		 * lblHardware.setEnabled(true); lblroot.setEnabled(true);
		 * lblReader.setEnabled(true);
		 * 
		 * // Power //if(true) { Reader2.GetPower(); //thread.sleep(300); try {
		 * Thread.sleep(300); } catch (InterruptedException e1) { // TODO Auto-generated
		 * catch block e1.printStackTrace(); } } //else { //Antenna state
		 * Reader2.GetAntennaState(); //Thread.sleep(300); try { Thread.sleep(300); }
	...
	private void SetConfiguration() {
		...
		if (Reader != null) {
			// if (reader.IsHandling && !reader.IsInventorying)
			{
				...
				int antstate = (chckbxAntena.isSelected() ? 1 : 0) + ((chckbxAntena_1.isSelected() ? 1 : 0) << 1)
						+ ((chckbxAntena_2.isSelected() ? 1 : 0) << 2) + ((chckbxAntena_3.isSelected() ? 1 : 0) << 3);
	...
	private void LockTag(String killpermissions, String accesspermissions, String epcpermissions, String tidpermissions,
			String userpermissions, String password) {
		...
		if (Reader != null) {
			// if (Reader.IsHandling)
			{
				...
				{
					mask = killpermissions + accesspermissions + epcpermissions + tidpermissions;
	...
	public class ReaderEventHandler implements IEventHandler<ReaderEventArgs> {
		...
		public void OnReaderEvent(Object sender, ReaderEventArgs e) {
			/*
			 * if(e.getpayload() != null) { String s = new String(e.getpayload());
			 * //System.out.println(s); System.out.println( "OnReaderEvent [" + e.getkind()
			 * + "] [" + e.getmessage() + "] [" + s + "]"); } else { System.out.println(
			 * "OnReaderEvent [" + e.getkind() + "] [" + e.getmessage() + "] [" +
			 * e.getpayload() + "]"); }
			 */
	...
	/*
	 * //�׽�Ʈ ���� public class ReaderEventHandler2 implements
	 * IEventHandler<ReaderEventArgs> {
	 * 
	 * ReaderApi reader2;
	 * 
	 * public ReaderEventHandler2(ReaderApi reader2){ this.reader2 = reader2;
	 * reader2.Visited.addEventHandler(this);
	 * 
	 * }
	 * 
	 * @Override public void OnReaderEvent(Object sender, ReaderEventArgs e) {
	 * 
	 * if(e.getpayload() != null) { String s = new String(e.getpayload());
	 * //System.out.println(s); System.out.println( "OnReaderEvent [" + e.getkind()
	 * + "] [" + e.getmessage() + "] [" + s + "]"); } else { System.out.println(
	 ...
	 * e.getpayload() + "]"); }
	 * 
	 * System.out.println("REDERIP = " + reader2.getIpAddr() ); switch(e.getkind())
	 * { case Connected: //GetConfiguration();
	 * 
	 * reader2.GetVersion(0);
	 * 
	 ...
	 * tabbedPane.setEnabled(true); lblstatusLabel.setText(e.getmessage());
	 * btnConnetion.setText("Disconnect");
	 * 
	 * config_btnRead.setEnabled(true); config_btnWrite.setEnabled(true);
	 * 
	 * 
	 * break; case Disconnected: tabbedPane.setEnabled(false);
	 * lblstatusLabel.setText(e.getmessage()); btnConnetion.setText("Connect");
	 * 
	 * 
	 * lblReader.setEnabled(false); lblHardware.setEnabled(false);
	 * lblroot.setEnabled(false);
	 * 
	 * chckbxAntena.setEnabled(false); chckbxAntena_1.setEnabled(false);
	 * chckbxAntena_2.setEnabled(false); chckbxAntena_3.setEnabled(false);
	 * 
	 * power.setEnabled(false);
	 * 
	 * config_btnRead.setEnabled(false); config_btnWrite.setEnabled(false);
	 * 
	 * //this.splitContainer1.Panel2.Enabled = false; break; case timeout: break;
	 * case Version:
	 * 
	 * String payload = new String(e.getpayload()); String items =
	 * payload.substring(1,3);
	 * 
	 * switch (items) { case "v0": // Reader Version Readerver =
	 * payload.substring(3,payload.length() - 2); break; case "v2": // Root Version
	 * Hardwarever = payload.substring(3,payload.length() - 2); break; case "v5": //
	 * H/W Version rootver = payload.substring(3,payload.length() - 2); break; }
	 * 
	 * lblReader.setText("* Reader : " + Readerver);
	 * lblHardware.setText("* Hardware :" + Hardwarever); lblroot.setText("root :" +
	 * rootver);
	 * 
	 * lblReader.setEnabled(true); lblHardware.setEnabled(true);
	 * lblroot.setEnabled(true);
	 * 
	 * break; case AntennaState: case Power: //payload =
	 * Encoding.ASCII.GetString(e.Payload); payload = new String(e.getpayload());
	 * 
	 * switch (payload.substring(1, 2)) { case "e": // Antenna State //int number =
	 * Convert.ToInt32(payload.substring(1)); //int antenastate =
	 * Integer.parseInt(payload.substring(2,3)); int antenastate =
	 * Integer.parseInt(payload.substring(2,payload.length() - 2));
	 * 
	 * System.out.println("antenastate = " + antenastate );
	 * 
	 * boolean checked1 = (antenastate & 0x0001) > 0 ? true:false; boolean checked2
	 * = (antenastate & 0x0002) > 0 ? true:false; boolean checked3 = (antenastate &
	 * 0x0004) > 0 ? true:false; boolean checked4 = (antenastate & 0x0008) > 0 ?
	 * true:false;
	 * 
	 * chckbxAntena.setSelected(checked1); chckbxAntena_1.setSelected(checked2);
	 * chckbxAntena_2.setSelected(checked3); chckbxAntena_3.setSelected(checked4);
	 * 
	 * chckbxAntena.setEnabled(true); chckbxAntena_1.setEnabled(true);
	 * chckbxAntena_2.setEnabled(true); chckbxAntena_3.setEnabled(true);
	 * 
	 * break;
	 * 
	 * case "p": //Power Value //this.nudPower.Value =
	 * Convert.ToInt32(payload.Substring(1)); //int powervalue =
	 * Integer.parseInt(payload.substring(2,5));
	 * 
	 * int powervalue = Integer.parseInt(payload.substring(2,payload.length() - 2));
	 * System.out.println("powervalue = " + powervalue );
	 * 
	 * power.setValue(powervalue); power.setEnabled(true); break; }
	 * 
	 * break; case TagId: // payload = Encoding.ASCII.GetString(e.Payload); payload
	 * = new String(e.getpayload()); //String[] tagIds = payload.Split(new String[]
	 * { "\r\n>" }, StringSplitOptions.RemoveEmptyEntries);
	 * 
	 * testcount++; //System.out.println("testcount = " + testcount ); TagIDtable =
	 * (DefaultTableModel)table.getModel(); TagIDtable.insertRow(0, new
	 * Object[]{payload,testcount +"  "+ reader2.getIpAddr()});
	 * 
	 * //table.updateUI();
	 * 
	 * 
	 ...
	 * 
	 * 
	 * //foreach (String tagid in tagIds) { //this.lvwInventory.Items.Add(tagid);
	 * 
	 * //String epc = tagid.Substring(2);
	 * 
	 * //this.lvwInventory.BeginUpdate();
	 ...
	 * //ListViewItem lvi = this.lvwInventory.Items.Insert(0, epc);
	 * 
	 * // String txt = String.Empty; // if (reader.TagType ==
	 * TagType.ISO18000_6C_GEN2) {
	 * 
	 * // int p = 4; // if (tagid.length > p) { // String hex = epc.Substring(p,
	 * epc.Length - p); // txt = reader.MakeTextFromHex(hex); } }
	 * //lvi.SubItems.Add(txt);
	 * 
	 * //this.lvwInventory.EndUpdate(); //this.lvwInventory.Refresh(); } break; case
	 * GetTagMemory: //payload = Encoding.ASCII.GetString(e.Payload); payload = new
	 * String(e.getpayload()); if (payload.substring(2, 3).equals("T") ||
	 * payload.substring(2, 3).equals("B")) {
	 * rw_tagid_textField.setText(payload.substring(3, payload.length() - 2));
	 * //rw_tagid_textField.setText(payload.substring(3,7) + "-" // +
	 * payload.substring(7,11) + "-" // + payload.substring(11,15) + "-" // +
	 * payload.substring(15,19) + "-" // + payload.substring(19,23) + "-" // +
	 * payload.substring(23,27));
	 * 
	 * } break;
	 * 
	 * case TagResponseCode: //payload = Encoding.ASCII.GetString(e.Payload);
	 * payload = new String(e.getpayload());
	 * 
	 * if (payload.substring(2, 3).equals("C")) { String code =
	 * payload.substring(3,5);
	 * 
	 * if(tabbedPane.getTitleAt(tabbedPane.getSelectedIndex()).equals(tabbedPane.
	 * getTitleAt(2))) { //rw_textPane.setText(code + " - " +
	 * Reader.Responses(code)); rw_txtarea.append(code + " - " +
	 * Reader2.Responses(code) + "\n");
	 * rw_txtarea.setCaretPosition(rw_txtarea.getDocument().getLength()); } else {
	 * //lk_txtarea.setText(code + " - " + Reader.Responses(code));
